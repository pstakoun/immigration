      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-5 space-y-6">
            ...
            {/* Optional: Existing Case (secondary flow) */}
            {typeof window !== "undefined" && (
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-gray-900">Already filed anything?</div>
                    <p className="text-xs text-gray-500">
                      Optional. Track an in-progress case for accurate remaining time + fewer wrong “you still need X” steps.
                    </p>
                  </div>
                  {onTrackCase && (
                    <button
                      type="button"
                      onClick={onTrackCase}
                      className="px-3 py-2 rounded-lg bg-white border border-gray-300 text-sm font-medium text-gray-800 hover:bg-gray-50 whitespace-nowrap"
                    >
                      {hasTrackedCase ? "Edit case" : "Set up tracker"}
                    </button>
                  )}
                </div>
